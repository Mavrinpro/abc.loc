...
class WPComment_Admin {
	...
	*/
	function add_comment_menu() {
	    ...
	    add_comments_page( __( 'Comment Fields', 'wpcomment' ), 
	    					__( 'Comment Fields', 'wpcomment' ), 
	...
	*/
	function comment_meta() {
		...
		echo '<div id="wpcomment-pre-loading"></div>';
		    
		echo '<div class="wpcomment-admin-wrap woocommerce wpcomment-wrapper" style="display:nonezz">';
		
		echo '<h1 class="wpcomment-heading-style">' . __ ( 'Create Comment Fields', "wpcomment" ) . '</h1>';
		echo '<p>' . __ ( 'Create different meta groups for different products.', "wpcomment" ) . '</p>';
	...
	*/
	...
	function save_meta_fields() {
		...
		// wp_send_json($wpcomment_meta);
		$wpcomment_meta = json_encode($wpcomment_meta);
		
		update_option('wpcomment_meta_fields', $wpcomment_meta);
		...
		if ($wpcomment_meta) {
			...
			$resp = array (
					'message'		=> __ ( 'Fields added successfully', 'wpcomment' ),
					'status'		=> 'success',
					'redirect_to'	=> $redirect_to,
			);
		} else {
			$resp = array (
					'message' => __ ( 'No changes found.', 'wpcomment' ),
					'status' => 'success',
			);
		}
	...
	function save_form_settings(){
		...
		$resp = array (
				'message'		=> __ ( 'Settings saved successfully', 'wpcomment' ),
				'status'		=> 'success',
		);
