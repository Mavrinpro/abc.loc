...
class WPComment_Form {
    ...
    function fields_render(){
        ...
        foreach( self::$fields as $meta ) {
            ...
            $title     = isset($meta['title']) ? wpcomment_wpml_translate($meta['title'], 'wpcomment') : '';
	...
	function get_field_default_value($posted_values, $data_name, $meta){
        ...
        }else if( isset($_POST['wpcomment']['fields'][$data_name]) && apply_filters('wpcomment_retain_after_add_to_cart', true) ) {
		    $default_value  = sanitize_text_field($_POST['wpcomment']['fields'][$data_name]);
	    } else {
