...
class WPComment_Frontend {
    ...
    function render_comment_meta_front( $comment, $context="front" ) {
    	
    	// if admin disable frontend extra fields
    	if( wpcomment_disable_extra_fields() ) return $comment;
        
        // $org_comment = $comment;
        $comment_meta_text = $this->get_comment_meta_by_comment_id(get_comment_ID());
		if($comment_meta_text){
			$comment .= $comment_meta_text;
		}
	...
	function comment_fields($comment_meta) {
		...
		if ( $comment_meta ) {
			...
			if ( $comment_meta_heading != '' ) {
				$comment .= sprintf(__('<p class="wpcomment-meta">%s</p>', 'wpcomment'), $comment_meta_heading);
			}
