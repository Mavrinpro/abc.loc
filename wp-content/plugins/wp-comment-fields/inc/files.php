...
function wpcomment_upload_file() {
	...
	if ( ! wp_verify_nonce( $wpcomment_nonce, $file_upload_nonce_action ) && apply_filters('wpcomment_verify_upload_file', true) ) {
		...
		$response ['message'] = __ ( 'You cannot upload the file at this time, please refresh the page and try again. Note that your current option choices will be reset.', 'wpcomment' );
	...
	if ($file_dir_path == 'errDirectory') {
		...
		$response ['status'] = 'error';
		$response ['message'] = __ ( 'Error while creating directory', 'wpcomment' );
		wp_send_json( $response );
	}
	...
	if( in_array( strtolower($extension), $restricted_type) ){
		$response ['status'] = 'error';
		$response ['message'] = __ ( 'File type not valid - '.$extension, 'wpcomment' );
		wp_send_json( $response );
	}
	/* ========== Invalid File type checking ========== */
...
function wpcomment_uploaded_file_preview($file_name, $settings){
  ...
  //      $html .= ob_get_clean();
		...
		$file_tools .= '<a href="#" class="nm-file-tools btn btn-primary u_i_c_tools_del" title="'.__('Remove', "wpcomment").'">'.__('Delete', 'wpcomment').'</span></a>';
